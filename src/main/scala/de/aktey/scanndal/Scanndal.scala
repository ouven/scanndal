...
import de.aktey.scanndal.classfile.{ClassFile, ClassFileReader}
...
class Scanndal(scanPackage: String = null) {
  ...
  else ucl.getResources(scanPackage.replace('.', '/')).asScala.toStream
  ...
  def scanFile(f: File): Stream[ClassFile] = {
    ...
    getClassFiles(f)
      ...
      .map((f: File) => new BufferedInputStream(new FileInputStream(f), 1024))
  ...
  def scanJar(jarFile: JarFile): Stream[ClassFile] = {
    ...
    jarFile.entries.asScala.toStream
