 ...
 */
class DeepTypeScan[A: Manifest](scanPackage: String = null) {
	val typeName = manifest[A].erasure.getCanonicalName
	...
	def scan: Iterable[ClassFile] = {
		...
		val extendingClasses = if (manifest[A].erasure.isInterface) {
